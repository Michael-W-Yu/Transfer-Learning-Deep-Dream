...
class DeepDream():
    ...
    def forward(self, img_tf, verbose=False):
        ...
        for i in self.selected_layer_inds:
            if self.filter_inds == []:
                average = tf.reduce_mean(netacts[i])
            else:
                #subset = tf.gather(tf.gather(netacts[i], self.filter_inds, axis = 1), self.filter_inds, axis = 2)
                subset = tf.gather(netacts[i], self.filter_inds, axis = 3)
                average = tf.reduce_mean(subset)
            average_na.append(average)
        return average_na
    ...
    def image_gradient(self, img_tf, eps=1e-8, normalized=True, verbose=False):
        ...
        grads = []
        for i in range(len(self.selected_layer_inds)):
            grad = tape.gradient(netActs[i], img_tf)
            grads.append(grad)
        if normalized==True:
            for j in range(len(grads)):
                grads[j] = grads[j] / (tf.math.sqrt((tf.reduce_mean(tf.math.square(grads[j])))) + eps)
    ...
    def gradient_ascent(self, img_tf, n_iter=10, step_sz=0.01, clip_low=0, clip_high=1, verbose=False):
        ...
        '''
        gradient = self.image_gradient(img_tf)
        for i in range(n_iter):
            for g in gradient:
                 img_tf.assign_add(step_sz*g)
#            img_tf.assign_add(step_sz*gradient[-1])
        return tf.clip_by_value(img_tf, clip_value_min = clip_low, clip_value_max = clip_high)
    ...
    def gradient_ascent_multiscale(self, img_tf, n_scales=4, scale_factor=1.3, n_iter=10, step_sz=0.01,
        ...
        img_tf: tf.Variable. shape=(img_y, img_x, n_chans). Input image modified via gradient ascent.
        '''
        base_shape = tf.cast(tf.shape(img_tf)[:-1], tf.float32)
        ...
        for k in range(n_scales):
            new_shape = tf.cast(base_shape*(scale_factor**(k+1)), tf.int32)
            img_tf = tf.Variable(tf.image.resize(img_tf, new_shape))
            img_tf = self.gradient_ascent(img_tf, n_iter = n_iter, step_sz=step_sz, clip_low=clip_low, clip_high=clip_high, verbose=verbose)
        return tf.clip_by_value(img_tf, clip_value_min = clip_low, clip_value_max = clip_high)
